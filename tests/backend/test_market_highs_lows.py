...
def test_high_list_includes_stocks_at_52w_high(hl_app):
    ...
    _series(bm, "ATHIGH", list(zip(D, [80.0, 90.0, 100.0])))   # latest 100 = window max → at high
    _series(bm, "MIDDLE", list(zip(D, [100.0, 60.0, 70.0])))   # latest 70, hi 100 → 30% below high
...
def test_low_list_includes_stocks_at_52w_low(hl_app):
    ...
    _series(bm, "ATLOW", list(zip(D, [80.0, 60.0, 50.0])))     # latest 50 = window min → at low
    _series(bm, "MIDDLE", list(zip(D, [40.0, 100.0, 70.0])))   # latest 70, lo 40 → 75% above low
