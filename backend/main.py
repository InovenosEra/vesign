...
def signals_today(signal: Optional[str] = None, market: Optional[str] = None,
    ...
    if signal and signal.upper() == "BUY":
        ...
        # can't be reconstructed client-side. MUST also run on the stable,
        # pre-live-overlay upside (below): the frontend polls this endpoint every
        # 3s and paginates a fixed-size window over the result, so sorting by a
        # value that's recomputed from the ticking live price every request causes
        # cards near a rank boundary to swap pages every poll — they'd appear to
        # flicker in and out of view even though nothing about the signal changed.
...
def unlock_signal(body: UnlockBody, user=Depends(get_current_user)):
    ...
    if scope == "tier":
        ...
        if kind == "open":
            # Flat sentinel, not one row per open trade — a single permanent
            # grant that also covers trades opened after this purchase.
            occurrences = [ent.OPEN_ALL_UNLOCK_KEY[1:]]
...
def portfolio_holdings(user=Depends(get_current_user), market: str = Query(default="US")):
    ...
    with engine.connect() as conn:
        ...
        meta = {r[0]: r for r in conn.execute(text(f"""
            ...
            LEFT JOIN company_health h ON c.ticker = h.ticker
            LEFT JOIN (
                SELECT ticker, signal, fair_value_upside, prediction_score, target_mean_price
                FROM (
                    SELECT ticker, signal, fair_value_upside, prediction_score, target_mean_price,
                           ROW_NUMBER() OVER (PARTITION BY ticker ORDER BY date DESC) AS rn
                    FROM signals
    ...
    for r in rows:
        ...
        m = meta.get(ticker)
        # Column order: ticker, company, logo_url, industry, domain, market_cap, latest_close,
        # prev_close, health_score, fair_value_upside, prediction_score, target_mean_price
        mc = m[5] if m else None
        hsc = m[8] if m else None
        fvu = m[9] if m else None
        pscore = m[10] if m else None
        tmp = m[11] if m else None
        sector = m[12] if m else None
        signal = m[13] if m else None
        ...
        result.append({
            ...
            "target_mean_price": float(tmp) if tmp is not None and not (isinstance(tmp, float) and math.isnan(tmp)) else None,
            "sector": sector,
            "signal": signal,
            "total_qty": total_qty,
