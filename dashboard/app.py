...
st.markdown(
    ...
    <style>
        ...
        /* Hide infrastructure containers once JS moves sticky header to <body> */
        ._st-infra-hide {
            display: none !important;
        }
        /* Search bar container: position:absolute removes from flex flow
           without hiding position:fixed children (search input / button) */
        ._st-infra-abs {
        ...
        /* ── Search bar pinned to sticky header ── */

        /* Collapse the original Streamlit row; position:fixed children escape overflow:hidden */
        div[data-testid="stHorizontalBlock"]:has(input[aria-label="Search Company or Ticker"]) {
            height: 0 !important;
        ...
        /* Fix the search input to the header centre.
           Header is 64px tall; input is ~40px → top = (64-40)/2 = 12px */
        div[data-testid="stTextInput"]:has(input[aria-label="Search Company or Ticker"]) {
        ...
        div[data-testid="stTextInput"]:has(input[aria-label="Search Company or Ticker"]) label {
            display: none !important;
        }

        /* Clear button: same top, same height as the input field → (64-40)/2 = 12px */
        div[data-testid="stHorizontalBlock"]:has(input[aria-label="Search Company or Ticker"]) button {
            position: fixed !important;
            top: 12px !important;
...
(function() {
  ...
  // ── Fix layout: remove infrastructure widgets from the flex flow ──
  // Streamlit's stVerticalBlock uses gap:1rem between flex items.
  // The 3 infrastructure items (sticky-header, this JS component, search bar)
  // each contribute 0px height but 1rem of gap, adding ~48px of dead space.
  // Fix: move #sticky-header to <body>, then hide all 3 containers.
  function getFlexParent(doc) {
    return doc.querySelector('[data-testid="stVerticalBlock"]') ||
           doc.querySelector('.block-container');
  }
  function getTopContainer(el, flexParent) {
    if (!el || !flexParent) return null;
    let cur = el;
    while (cur && cur.parentElement !== flexParent) {
      if (!cur.parentElement || cur.parentElement === window.parent.document.body) return null;
  ...
  function fixLayout() {
    ...
    const flexParent = getFlexParent(doc);
    if (!flexParent) return;

    // 1. Sticky-header container → move header to <body>, then display:none
    const sticky = doc.getElementById("sticky-header");
    if (sticky) {
      const stickyBox = getTopContainer(sticky, flexParent);
      if (sticky.parentElement !== doc.body) {
        doc.body.appendChild(sticky);
      }
      if (stickyBox && stickyBox !== flexParent) {
        stickyBox.classList.add("_st-infra-hide");
      }
    }

    // 2. This component's container → display:none (iframe JS still runs)
    const iframe = window.frameElement;
    if (iframe) {
      const compBox = getTopContainer(iframe, flexParent);
      if (compBox && compBox !== flexParent) {
        compBox.classList.add("_st-infra-hide");
      }
    }

    // 3. Search bar container → position:absolute (keeps position:fixed children visible)
    const searchInput = doc.querySelector('input[aria-label="Search Company or Ticker"]');
    if (searchInput) {
      const searchBox = getTopContainer(searchInput, flexParent);
      if (searchBox && searchBox !== flexParent) {
        searchBox.classList.add("_st-infra-abs");
      }
    }
  }

  fixLayout();
  // Re-run after React may have re-rendered
...
@st.cache_data(ttl=60)
def watchlist_signal_data(tickers_tuple):
    tickers = list(tickers_tuple)
    if not tickers:
        return pd.DataFrame()
    ticker_list = "','".join(tickers)
    # JOIN on per-ticker max date (computed once, not a correlated subquery per row)
    ...
    if _selected_list:
        ...
        if _list_id is not None:
            ...
            if wl_df.empty:
                ...
                try:
                    signal_data = watchlist_signal_data(tuple(wl_df["ticker"].tolist()))
                    ...
                    desired_cols = ["logo_url", "ticker", "company", "signal",
                                    "close", "rsi", "fair_value_upside", "target_mean_price",
